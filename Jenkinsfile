pipeline {
    ...
    environment {
        ...
        CONTAINER_PORT = '80'
        HOST_PORT = '8080'  // Change this to your desired host port
    }
    ...
    stages {
        ...
        stage('Prepare nginx.conf') {
            steps {
                sh '''
                    # Check if backend nginx.conf exists, if not create a basic one
    ...
    post {
        ...
        success {
            echo "Pipeline completed successfully! Frontend is deployed at http://${DEPLOY_HOST}:${HOST_PORT}"
        }
        failure {
