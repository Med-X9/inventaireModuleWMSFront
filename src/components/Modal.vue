...
<template>
    <div v-if="modelValue" class="fixed  inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center  px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block p-6 align-bottom panel rounded-lg text-left shadow-xl transform transition-all
              sm:my-8 sm:align-middle sm:max-w-3xl max-w-3xl w-full
              " role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                <button type="button"
                    class="absolute top-3 right-3 text-gray-500 hover:text-gray-700 focus:outline-none" @click="close"
                ...
                <div class="sm:flex mt-6 sm:items-start">
                    <div class="mt-4 text-center sm:mt-0 sm:text-left w-full">
                        <h3 v-if="title" class="text-lg leading-6 font-medium text-gray-900 mb-6 mt-2"
                            id="modal-headline">
                            {{ title }}
                        </h3>
                        <div class="mt-2">
