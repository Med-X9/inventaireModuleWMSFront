<template>
    <div>
        ...
        <div v-if="rowData !== undefined" class="table-container">
            <ag-grid-vue class="ag-theme-alpine" :style="dynamicGridStyle" domLayout="normal" :theme="gridTheme"
                @grid-ready="onGridReady" @first-data-rendered="onFirstDataRendered"
                @selection-changed="onSelectionChanged" @row-clicked="$emit('row-clicked', $event)"
                @model-updated="onModelUpdated" @cell-value-changed="onCellValueChanged"
            ...
            cellStyle: (params) => params.data?.isChild ? { display: 'none' } : undefined
                ...
                :stopEditingWhenCellsLoseFocus="false" @cellKeyDown="onCellKeyDown"
