    ...
    <div>
        <nav v-if="items && items.length > 0"
            ...
            <div class="flex-1 min-w-0">
                <ul
                    class="flex items-center space-x-1 rtl:space-x-reverse overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                    <li v-for="(item, index) in items" :key="index" class="flex items-center flex-shrink-0">
                        ...
                        <span v-if="index > 0" class="mx-1 sm:mx-2 text-gray-400 flex-shrink-0">
                            /
                        </span>

                        <!-- Élément avec tooltip pour INV-001 -->
                        <div v-if="item.label === 'INV-001'" class="relative group cursor-pointer flex-shrink-0"
                            v-tippy="{
                        ...
                        <router-link v-else-if="item.path && !item.isActive" :to="item.path"
                        ...
                        <span v-else class="whitespace-nowrap" :class="{
            ...
            <div v-if="shouldShowSteps && inventorySteps.length > 0" class="flex-shrink-0">
                <div class="flex items-center justify-start lg:justify-end">
                    <div
                        class="flex items-center space-x-1 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                        <div v-for="(step, index) in inventorySteps" :key="step.id"
                            class="flex items-center flex-shrink-0">
                            <!-- Étape -->
                            <div class="flex items-center">
                                <div class="flex items-center justify-center w-6 h-6 rounded-full transition-all duration-200 text-xs flex-shrink-0"
                                    :class="{
                                    ...
                                    <svg v-if="step.isCompleted" class="w-3 h-3" fill="currentColor"
                                        viewBox="0 0 20 20">
                                ...
                                <span class="ml-2 text-xs sm:text-sm font-medium whitespace-nowrap" :class="{
                            ...
                            <div v-if="index < inventorySteps.length - 1"
