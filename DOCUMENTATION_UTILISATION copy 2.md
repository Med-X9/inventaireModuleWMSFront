...
### Stepper de Formulaire (Multi‑Étapes)

Exemple de formulaire multi‑étapes (stepper) en utilisant `Form` et `FormGroup` du design system.
...
const validateStep = (step: number): boolean => {
  ...
  if (step === 1) {
    if (!form.value.firstName) errors.value.firstName = 'Prénom obligatoire'
    if (!form.value.lastName) errors.value.lastName = 'Nom obligatoire'
  }

  if (step === 2) {
    if (!form.value.email) errors.value.email = 'Email obligatoire'
  }

  // Étape 3 : validation simple d'exemple
  if (step === 3) {
    if (!form.value.address) errors.value.address = 'Adresse obligatoire'
  }
...
const handleSubmit = () => {
  ...
  // Ici, toutes les étapes sont supposées valides
  console.log('Formulaire stepper complet :', form.value)
}
</script>
```
...
Le DataTable est le composant principal pour afficher des données tabulaires. Pour les grandes listes, le virtual scrolling (`enableVirtualScrolling`, `virtualScrollingConfig`) et les hauteurs variables (`getRowHeight`) sont documentés dans **DOCUMENTATION_DATATABLE.md**.
